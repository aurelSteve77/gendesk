...
Five claims from the Netflix posts were worth testing rather than repeating.
**Two replicated, one did not, one produced a clean null, and one holds only in some
regimes** -- all of them are written up in [`docs/findings.md`](docs/findings.md),
...
- **Hybrid row decoding replicated cleanly**: 39% lower median decode latency and 15 of
  41 sequential model invocations removed per page.
- **The advantage is regime-dependent.** In the validation window's COVID crash and
  recovery, the raw teacher screen edges the model out. A single test window would have
  hidden that.
